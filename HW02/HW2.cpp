...
void calulate(vector<Variable>& variableList, const string infix) { 
    ...
    if (!infixToVector(infix, vector_infix, variableList)) {                   // 沒有輸入合法的表達式 
    ...
    InfixToPostfix(vector_infix, postfix); // 將中序換後序

    Stack calcStack; // 儲存計算結果的 Stack
    ...
    for (const auto& tok : postfix) { 
        if (tok.type == 0) { // 如果是數字，則直接push到Stack
            ...
            if (op == "!") {                                                       // 處理負號
                ...
                if (op == "AND") result = left && right;     // AND
                else if (op == "OR") result = left || right; // OR
